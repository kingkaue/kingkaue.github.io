...
class MediaGallery {
  ...
  init() {
    if (!this.container) {
      console.error(
        `Gallery container with ID '${this.containerId}' not found`
      );
      return;
    }

    console.log(
      `Initializing gallery: ${this.container.id} with ${this.galleryData.length} items`
    );
  ...
  createGalleryItems() {
    this.galleryData.forEach((item, index) => {
      ...
      if (item.type === "image") {
        ...
        mediaContent = `<img src="${item.src}" alt="${item.title}" class="gallery-gif">`;
      }

      // Add caption to gallery item
  ...
  goToSlide(index) {
    ...
    // Show selected slide
    this.galleryMain
      .querySelector(`.gallery-item[data-index="${index}"]`)
  ...
  setupEventListeners() {
    ...
    this.galleryMain.addEventListener(
      "touchstart",
      (e) => {
...
function initAllGalleries() {
  ...
  galleryContainers.forEach((container) => {
    ...
    console.log(
      `Gallery ID: ${galleryId}, Looking for data: ${expectedDataVar}`
    );
    console.log("Available data:", galleryData);

    if (galleryData && Array.isArray(galleryData)) {
      console.log(
        `Initializing gallery ${galleryId} with ${galleryData.length} items`
      );
      new MediaGallery(galleryId, galleryData);
    } else {
      console.warn(
        `No gallery data found for ${galleryId}. Expected variable: ${expectedDataVar}`
      );
    }
  });
}
